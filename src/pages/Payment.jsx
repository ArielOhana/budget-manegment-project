...
const handleMonthOnCardChange = (e) => {
  ...
  const handleCreditCardNumberChange = (e) => {
    ...
    setIsValidCreditCard(validateCreditCard(input));
  };

  const validateCvvCode = (input) => {
    const regex = /^[0-9]{3}$/;
    return regex.test(input);
  };
  ...
  const handleCvvCodeChange = (e) => {
    const input = e.target.value;
    setCvvCode(input);
    setIsValidCvv(validateCvvCode(input));
  };
